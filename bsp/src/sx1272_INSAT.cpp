...
*/
uint8_t sx1272_INSAT::readRegister(uint8_t address)
{
	uint8_t var;
...
int8_t sx1272_INSAT::setMode(uint8_t mode)
{
	...
	if (state == 0)
	{
		my_printf(F("## Mode "));
		my_printf(mode, DEC);
		my_printf(F(" configured with success ##"));
	}
	else
	{
		my_printf(F("** There has been an error while configuring mode "));
		my_printf(mode, DEC);
		my_printf(F(". **"));
	}
#endif
    ...
    */
		...
		{ // packet received
			...
			{ // CRC correct
				...
				p_received = true;
				#if (SX1272_debug_mode > 0)
					my_printf(F("## Packet correctly received in FSK mode ##"));
				#endif
			}
			else
			{ // CRC incorrect
				...
				p_received = false;
				#if (SX1272_debug_mode > 0)
					my_printf(F("## Packet incorrectly received in FSK mode ##"));
				#endif
			}
		}
		else
...
float sx1272_INSAT::max(float argument1, float argument2)
{
